...
function renderSign(){const s=signs[current];$("#signCard").src=`assets/sign-${String(s.m).padStart(2,"0")}.webp`;$("#signName").textContent=s.name;$("#meta").textContent=`${s.month}｜${s.reading}｜${s.icon} ${s.el}`;$("#catch").textContent=`― ${s.catch} ―`;$("#talent").textContent=s.talent;$("#weak").textContent=s.weak;$("#love").textContent=s.love;$("#work").textContent=s.work;$("#pair").textContent=`${signs[s.pair-1].name}（${s.pair}月）`;$("#action").textContent=s.action;$("#motto").textContent=s.motto}
